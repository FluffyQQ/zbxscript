...
def send_telegram_alert(
    ...
    caption = f"""<b>Ошибка в тесте iTrans "Модуль управления ЖД плечом"</b>\n\n<b>Шаг:</b> {step_info}\n<b>Дата:</b> {date_str}\n<b>Время:</b> {time_str}"""
    ...
    for attempt in range(max_retries + 1):
        ...
        try:
            ...
            if attempt < max_retries:
                ...
                save_screenshot_file(driver, 'itrans_zd_error.png')
...
def main() -> int:
    ...
    try:
        ...
        if not execute_step_with_retry(
            ...
            driver, test_result, telegram_token, telegram_chat_id,
            step_description="Вкладка Модуль управления ЖД плечом",
        ):
            return 1
        ...
        if not execute_step_with_retry(
            ...
            driver, test_result, telegram_token, telegram_chat_id,
            step_description="Вкладка Отчеты",
        ):
            return 1
        ...
        if not execute_step_with_retry(
            ...
            driver, test_result, telegram_token, telegram_chat_id,
            step_description="Вкладка Единое окно",
        ):
            return 1
