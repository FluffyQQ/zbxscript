...
def send_telegram_alert(
    ...
    caption = f"""<b>Ошибка в тесте iTrans "Модуль управления морским плечом"</b>\n\n<b>Шаг:</b> {step_info}\n<b>Дата:</b> {date_str}\n<b>Время:</b> {time_str}"""
...
def step_01_open_and_login(
    test: ItransTest, base_url: str, username: str, password: str
) -> None:
    d = test.driver
    try:
        ...
        if resp.status_code != 200:
            raise Exception(
                f"Ошибка при открытии сайта: HTTP статус = {resp.status_code}"
            )
    except Exception:
        raise Exception(
            "Ошибка при открытии сайта: не удалось получить HTTP 200 от сервера"
        )

    d.get(base_url)
    WebDriverWait(d, DEFAULT_WAIT).until(
    ...
    try:
        ...
        if "itrans.trcont.ru" not in current_url:
            raise Exception(
                f"Ошибка при открытии сайта: текущий URL = {d.current_url}"
            )
    except Exception:
        raise Exception(f"Ошибка при открытии сайта: текущий URL = {d.current_url}")

    # Нажатие кнопки "Модуль управления морским плечом"
    test.click_element("//*[@id='app']/div/div[3]/a[3]/span", timeout=30)
    ...
    # Проверка успешного входа — наличие вкладки "Sea Module"
    WebDriverWait(d, VISIBILITY_WAIT).until(
        EC.presence_of_element_located((By.XPATH, "//*[@id='accordion2']/button/span"))
    )


def step_02_open_sea_module(test: ItransTest) -> None:
    ...
    # Проверяем наличие вкладки: Sea Module
    tab_xpath = "//*[@id='accordion2']/button/span"
    if not test.element_is_present(tab_xpath, timeout=DEFAULT_WAIT):
        raise Exception("Вкладка 'Sea Module' не найдена на странице")
    test.click_element(tab_xpath, timeout=CLICK_WAIT, description="Sea Module")
    time.sleep(2)

    # Подвкладки и ожидаемые надписи на странице после открытия
    subtabs = [
        ("//*[@id='order-collapse']/ul/li[1]/a", "Line", "Line name"),
        ("//*[@id='order-collapse']/ul/li[2]/a", "Vessel", "Vessel Name"),
        ("//*[@id='order-collapse']/ul/li[3]/a", "Vessel schedule", "Line name"),
        ("//*[@id='order-collapse']/ul/li[4]/a", "Orders list", "Services"),
        ("//*[@id='order-collapse']/ul/li[5]/a", "Request/Booking", "Services"),
        ("//*[@id='order-collapse']/ul/li[6]/a", "Confirmation of loading", "ETA POL from"),
        ("//*[@id='order-collapse']/ul/li[7]/a", "Confirmation of discharge", "ETA POD from"),
    ]
    ...
    for attempt in range(max_retries + 1):
        ...
        try:
            ...
            if attempt < max_retries:
                ...
                send_telegram_alert(
                    telegram_token, telegram_chat_id, text=err, screenshot_b64=ss_b64, step_number=str(step_number), step_description=step_description
                )
...
def main() -> int:
    ...
    try:
        ...
        # Шаг 1 — Логин и вход в модуль управления морским плечом
        ...
        if not execute_step_with_retry(
            2,
            "step_02_open_sea_module",
            lambda: step_02_open_sea_module(test),
            driver, test_result, telegram_token, telegram_chat_id,
            step_description="Вкладка Sea Module",
        ):
            return 1
